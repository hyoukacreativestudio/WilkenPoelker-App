...
const Cleaning = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Rabatte laden
        const res = await axios.get('http://192.168.178.24:5000/api/products/cleaning/discounts', {
          headers: { Authorization: token }
        });
        setDiscounts(res.data);

        // Berechtigung prüfen (full_access / full_access_grant / cleaning)
        const hasPermission = user.permissions?.some(p => ['full_access', 'full_access_grant', 'cleaning'].includes(p));
  ...
  const askAI = async () => {
    ...
    try {
      const token = await SecureStore.getItemAsync('token');
      const res = await axios.post('http://192.168.178.24:5000/api/products/cleaning/askAI', { question }, {
        headers: { Authorization: token }
      });
  ...
  return (
    <View style={[styles.container, isDark && styles.containerDark]}>
      ...
      <View style={styles.content}>
        <Text style={[styles.title, isDark && styles.textDark]}>Aktuelle Rabatte – Reinigungsgeräte</Text>
        ...
        {showAI && (
          <View style={[styles.aiBox, isDark && styles.boxDark]}>
            <TextInput
              ...
              placeholder="Frage zu Reinigungsgeräten..."
              value={question}
        ...
        {canPostOffer && (
          <View style={[styles.offerBox, isDark && styles.boxDark]}>
            <Text style={[styles.offerTitle, isDark && styles.textDark]}>Neues Angebot posten (Reinigungsgeräte)</Text>
            <TextInput
