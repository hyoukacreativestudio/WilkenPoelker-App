...
const Bikes = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Rabatte laden
        const res = await axios.get('http://192.168.178.24:5000/api/products/bikes/discounts', {
          headers: { Authorization: token }
        });
        setDiscounts(res.data);

        // Berechtigung prüfen (z. B. full_access oder bike)
        const hasPermission = user.permissions?.some(p => ['full_access', 'full_access_grant', 'bike'].includes(p));
  ...
  const askAI = async () => {
    ...
    try {
      const token = await SecureStore.getItemAsync('token');
      const res = await axios.post('http://192.168.178.24:5000/api/products/bikes/askAI', { question }, {
        headers: { Authorization: token }
      });
  ...
  return (
    <View style={[styles.container, isDark && styles.containerDark]}>
      ...
      <View style={styles.content}>
        <Text style={[styles.title, isDark && styles.textDark]}>Aktuelle Rabatte – Fahrräder</Text>
        ...
        {showAI && (
          <View style={[styles.aiBox, isDark && styles.boxDark]}>
            <TextInput
              ...
              placeholder="Frage zu Fahrrädern / E-Bikes / Leasing..."
              value={question}
        ...
        {canPostOffer && (
          <View style={[styles.offerBox, isDark && styles.boxDark]}>
            <Text style={[styles.offerTitle, isDark && styles.textDark]}>Neues Angebot posten</Text>
            <TextInput
