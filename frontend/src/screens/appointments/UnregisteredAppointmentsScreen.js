...
export default function UnregisteredAppointmentsScreen({ navigation }) {
  ...
  const {
    ...
    refresh,
    setItems,
  } = usePagination(appointmentsApi.getUnregisteredAppointments);
  ...
  const handleRegister = useCallback(async (appointmentId) => {
    const doRegister = async () => {
      try {
        setRegisteringId(appointmentId);
        await appointmentsApi.registerAppointment(appointmentId);
        setItems((prev) => prev.filter((a) => (a._id || a.id) !== appointmentId));
    ...
    return (
      <View style={{ marginHorizontal: theme.spacing.xs, marginBottom: theme.spacing.md, maxWidth: 600, alignSelf: 'center', width: '100%' }}>
        ...
        <Button
          ...
          onPress={() => handleRegister(itemId)}
          variant="primary"
          fullWidth
          loading={registeringId === itemId}
          icon={<MaterialCommunityIcons name="calendar-check" size={18} color="#FFFFFF" />}
          style={{ marginTop: theme.spacing.xs }}
        />
      </View>
    );
  }, [handlePress, handleRegister, registeringId, theme, t]);
  ...
  const renderEmpty = () => {
    ...
    return (
      <EmptyState
        ...
        title={t('appointments.noUnregistered')}
        message={t('appointments.noUnregisteredMessage')}
      />
    );
  };
